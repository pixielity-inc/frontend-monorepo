...
function extractNamespace(fileName: string, languageCode: string): string | undefined {
  const languageIndex = fileName.toLowerCase().indexOf(languageCode.toLowerCase());

  if (languageIndex === -1) {
    return undefined;
  }

  // Strip trailing dots/dashes left over after removing the language code
  let namespace = fileName
    .substring(0, languageIndex)
    .replace(/[.-]+$/, '')
    .trim();
