...
All notable changes to `@abdokouta/react-i18n` will be documented in this file.
...
- **DI Module** — `I18nModule.forRoot()` following the `HttpModule` /
  `MultiTenancyModule` pattern from `@abdokouta/ts-container`
...
- **Interface naming convention** — standardized on `I` prefix (`II18nService`,
  ...
  `@abdokouta/react-multitenancy` patterns
...
- Virtual module (`virtual:@abdokouta/react-i18n`)
