...
export interface MultitenancyConfig {
   ...
   * This header will be added to all authenticated requests
   *
   * @default "X-Tenant-ID"
   ...
   * Called before each authenticated request
   *
   ...
   *
   * @param tenantId - New tenant ID to set
   *
   * @example
   * ```tsx
   ...
   * Use this to clear cached data, refresh resources, etc.
   *
   * @param tenantId - New tenant ID
   * @param previousTenantId - Previous tenant ID (if any)
   *
   * @example
   * ```tsx
   * onTenantChange: (tenantId, previousTenantId) => {
   *   console.log(`Switched from ${previousTenantId} to ${tenantId}`);
   *   // Clear cached data
   *   queryClient.clear();
   * }
   * ```
   ...
   * When true, tokens are stored per-tenant
   *
   * @default true
   *
   * @example
   * // With tenantAwareStorage: true
   * // Token key: "auth_token_tenant_123"
   *
   * // With tenantAwareStorage: false
   ...
   * Use this if your API returns tenant ID after login
   *
   * @param response - Login response data
   * @returns Tenant ID from response
   *
   * @example
   * ```tsx
   * extractTenantFromResponse: (response) => {
   *   return response.user.tenantId;
   * }
   * ```
   */
   ...
   * When true, auth check fails if no tenant is set
   *
   * @default false
   */
