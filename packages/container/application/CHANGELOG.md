...
- 🔍 `get()` / `getOptional()` / `has()` for provider resolution across all modules
- 🎯 `select(module, token)` for module-scoped provider resolution
- 🔄 Full lifecycle support: `onModuleInit()` on bootstrap, `onModuleDestroy()` on `close()`
- ⚛️ Implements `ContainerResolver` interface for `<ContainerProvider>` compatibility
